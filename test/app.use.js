...
describe("app", function () {
  it('should emit "mount" when mounted', function (done) {
    ...
    blog.on("mount", function (arg) {
      ...
      console.log("arg is app", arg === app);
      done();
    });
  ...
  describe(".use(app)", function () {
    ...
    it("should set the child's .parent", function () {
      ...
      console.log("blog.parent is app", blog.parent === app);
    });
    ...
    it("should support dynamic routes", function (done) {
      ...
      request(app).get("/post/twice-upon-a-time").expect("success", done);
    });
  ...
  describe(".use(path, middleware)", function () {
    ...
    it("should support empty string path", function (done) {
      ...
      request(app)
        .get("/")
        .expect(200, "saw GET / through /", (err) => {
          if (err) done(err);
        });
      // request(app).get("/foo").expect(200, "saw GET /foo through /foo", done);
      request(app)
        .get("/foo/bar/baz123")
        .expect(
          200,
          "saw GET /foo/bar/baz123 through /foo/bar/baz123",
          (err, res) => {
