...
describe("app.router", function () {
  it("should restore req.params after leaving router", function (done) {
    ...
    function handler1(req, res, next) {
      console.log("handler1", req.params.id);
      res.setHeader("x-user-id", String(req.params.id));
      next();
    }

    function handler2(req, res) {
      console.log("handler2", req.params.id);
  ...
  describe("decode params", function () {
    it("should decode correct params", function (done) {
      ...
      request(app).get("/foo%25bar").expect("foo%bar", done);
    });
  ...
  it("should be .use()able", function (done) {
    ...
    app.use(function (req, res, next) {
      calls.push("after");
      calls.push({ final: "fuck" });
      res.json(calls);
    });

    request(app)
      .get("/")
      .expect(200, ["before", "GET /", "after", { final: "fuck" }], done);
  });
  ...
  describe("when given a regexp", function () {
    it("should match the pathname only", function (done) {
      ...
      request(app).get("/user/12?foo=bar&baz=qux").expect("user", done);
    });
    ...
    it("should populate req.params with the captures", function (done) {
      ...
      request(app)
        .get("/user/10/edit")
        .expect("editing user 10", (err) => {
          if (err) done(err);
        });
      request(app).get("/user/20/view").expect("viewing user 20", done);
    });
  ...
  describe("params", function () {
    it("should overwrite existing req.params by default", function (done) {
      ...
      app2.use("/user/:user", (req, res) => {
        res.send(req.params);
      });

      request(app2)
        .get("/user/1/get")
        .expect(200, '{"user":"1"}', (err, res) => {
    ...
    it("should allow merging existing req.params", function (done) {
      ...
      request(app)
        .get("/user/tj/get")
        .expect(200, '[["action","get"],["user","tj"]]', (err, res) => {
          console.log(res.text);
          done();
        });
    });
  ...
  it('should allow literal "."', function (done) {
    ...
    app.get("/api/users/:from..:to", function (req, res) {
      console.log("from-to", req.params);
  ...
  describe("*", function () {
    it("should capture everything", function (done) {
      ...
      app.get("*", function (req, res) {
        console.log("capture-everything", req.params);
  ...
  describe("when next(err) is called", function () {
    ...
    it("should call handler in same route, if exists", function (done) {
      ...
      request(app).get("/foo").expect("error!", done);
    });
  });
