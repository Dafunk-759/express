...
describe("OPTIONS", function () {
  ...
  it("should only include each method once", function (done) {
    ...
    request(app)
      .options("/users")
      .expect("Allow", "GET,HEAD,PUT,POST")
      .expect(200, "GET,HEAD,PUT,POST", done);
  });
  ...
  describe("when error occurs in response handler", function () {
    it("should pass error to callback", function (done) {
      ...
      request(app).options("/users").expect(200, done);
    });
  });
});
...
describe("app.options()", function () {
  it("should override the default behavior", function (done) {
    ...
    request(app).options("/users").expect("GET").expect("Allow", "GET,POST", done);
  });
});
